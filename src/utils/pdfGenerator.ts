...
export const savePDF = (items: InventoryItem[]) => {
  ...
  doc.text(`Pawon Salam Resto  ·  Tanggal Cetak: ${new Date().toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' })}`, 14, 27);
  ...
  const renderTable = (category: 'Kitchen' | 'Mini Bar', startY: number) => {
    ...
    autoTable(doc, {
      ...
      didDrawPage: (data) => {
        ...
        doc.text('Inventory Asset Pawon Salam Resto', marginLeft, pageH - 9);
